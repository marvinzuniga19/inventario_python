...
class Usuario(UserMixin, db.Model):
    ...
    rol = db.Column(
        db.Enum("admin", "empleado", name="rol_usuario"), default="empleado"
    )
    ...
    moneda_preferida_id = db.Column(
        db.Integer, db.ForeignKey("monedas.id"), default=None
    )

    # Tema preferido del usuario
    tema_preferido = db.Column(
        db.String(20), default="default"
    )  # default, blue_ocean, green_forest, etc.
    sidebar_config = db.Column(
        db.JSON, default={}
    )  # Configuración personalizada de sidebar

    # Relaciones
    movimientos = db.relationship("Movimiento", backref="usuario", lazy=True)
    moneda_preferida = db.relationship(
        "Moneda",
        foreign_keys=[moneda_preferida_id],
        back_populates="usuarios_preferencia",
    )

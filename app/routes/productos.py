...
def lista():
    ...
    form.categoria.choices = [(0, "Todas")] + [
    ...
    valor_total = sum(
        p.get_valor_total() for p in Producto.query.filter_by(activo=True).all()
    )
...
def nuevo():
    ...
    if form.validate_on_submit():
        ...
        # Obtener moneda NIO por defecto
        ...
        producto = Producto(
            ...
            moneda_original_id=moneda_nio.id if moneda_nio else None,
            precio_original=form.precio.data,
            precio_base=form.precio.data,
...
def editar(id):
    ...
    if form.validate_on_submit():
        ...
        if form.imagen.data and allowed_file(form.imagen.data.filename):
            ...
            if producto.imagen_url:
                old_path = os.path.join(
                    current_app.config["UPLOAD_FOLDER"], producto.imagen_url
                )
        ...
        producto.moneda_original_id = moneda_nio.id if moneda_nio else None
        producto.precio_original = form.precio.data
        producto.precio_base = form.precio.data
    ...
    return render_template(
        "productos/form.html", form=form, producto=producto, titulo="Editar Producto"
    )
